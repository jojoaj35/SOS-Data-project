...
#Load dataset
file_path = r"C:\Users\Telet\Downloads\Correct UTSA Client Dataset - Students of Service (SOS).xlsx"
...
survey_raw = pd.read_excel(file_path, sheet_name='Likert Scale')
...
dashboard_layout = [
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardHeader([
                    html.H6("Senior Survey Responses", className="mb-0 text-center", style={'color': '#34495e', 'fontWeight': '600'})
                ]),
                dbc.CardBody([
                    dcc.Dropdown(
                        id='likert-question-drop',
                        options=[{'label': survey_raw.columns[i], 'value': survey_raw.columns[i]}for i in [0,18,19]],
                        value=survey_raw.columns[0],  # Default to first question column
                        style={'marginBottom': '10px'}
                    ),
                    dcc.Graph(id='likert-pie', style={'height': '300px'})
                ])
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    html.Div([
                        ...
                        html.Small("*Generated by STUDENTS since 2018*", style={'fontSize': '14px', 'color': '#27ae60'})
    ...
    # Remove the old row with the Senior Survey Responses Scooby Doo card
    # ...existing code...
]
...
# 1. Add to analytics_subtab_layout
analytics_subtab_layout = [
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardHeader([
                    html.H5("Pie Chart Visualization", className="mb-0 text-center", style={'color': '#34495e', 'fontWeight': '600'})
                ]),
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
...
@callback(
    Output('likert-pie', 'figure'),
    Input('likert-question-drop', 'value')
)
def update_likert_pie(selected_question):
    likert_order = ["Strongly Agree", "Agree", "Neutral", "Disagree", "Strongly Disagree"]
    counts = survey_raw[selected_question].value_counts().reindex(likert_order, fill_value=0)
    fig = px.pie(
        names=counts.index,
        values=counts.values,
    )
    fig.update_traces(textinfo='percent+label')
    fig.update_layout(margin=dict(l=20, r=20, t=40, b=20), height=300, font=dict(size=12)) #font size for likert scale key and questions
    return fig
...
# 2. Add callback for the pie chart
@callback(
...
def update_analytics_pie(pop_col, feature_col):
    yes_only_map = {
    ...
    if feature_col in yes_only_map:
        col = yes_only_map[feature_col]
        filtered = clients[clients[col] == 1]
        grouped = filtered.groupby(pop_col)['Galaxy ID'].count().reset_index()
        grouped.rename(columns={'Galaxy ID': 'Count'}, inplace=True)
        total = grouped['Count'].sum()
        grouped = grouped[grouped['Count'] >= 0.02 * total]
        fig = px.pie(
            grouped,
            names=pop_col,
            values='Count',
            title=f"{feature_col} by {pop_col}",
        )
    else:
        grouped = clients.groupby(pop_col)[feature_col].sum().reset_index()
        total = grouped[feature_col].sum()
        grouped = grouped[grouped[feature_col] >= 0.02 * total]
        fig = px.pie(
            grouped,
            names=pop_col,
            values=feature_col,
    ...
    fig.update_traces(textinfo='percent+label')
    fig.update_layout(margin=dict(l=20, r=20, t=40, b=20), height=350, font=dict(size=12))
    return fig
