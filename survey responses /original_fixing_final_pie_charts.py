            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Dropdown(
                        ...
                        options=[{'label': survey_raw.columns[i], 'value': survey_raw.columns[i]} for i in [0, 18, 19] if i < len(survey_raw.columns)],
                        value=survey_raw.columns[0] if not survey_raw.empty else None,
...
def update_likert_pie(selected_question):
    ...
    if not selected_question or selected_question not in survey_raw.columns:
        return px.pie(title="No data available. Please upload a dataset with a 'Likert Scale' sheet.")
    ...
    fig.update_traces(textinfo='percent+label')
    fig.update_layout(margin=dict(l=20, r=20, t=40, b=20), height=300, font=dict(size=12))
    return fig
